...
//       setTestimonials(prev => prev.filter(t => t.testId !== testId));
//     } catch (err) {
...
//         if (!t.clientName || !t.des) {
//           setError(`Testimonial #${idx + 1} is missing required fields.`);
//           setLoading(false);
//           return;
//         }
...
//           let resp;
//           if (t.testId) {
//             resp = await updateTestimonial(t.testId, testimonialData);
//             console.log('Update response:', resp);
//           } else {
//             resp = await addTestimonial(testimonialData);
//             console.log('Add response:', resp);
//           }
//         } catch (apiErr) {
//           console.error('API error:', apiErr);
//           setError(`Failed to save testimonial #${idx + 1}: ${apiErr?.response?.data?.message || apiErr.message || 'Unknown error'}`);
//           setLoading(false);
//           return;
//         }
//       }

//       // Refetch testimonials to get new testId and updated list
//       try {
//         const response = await getTestimonials();
...
//         } else if (data && Array.isArray(data.testimonials)) {
//           updatedData = data.testimonials;
//         }
//         setTestimonials(updatedData);
//         setImageFiles({});
//         setIsEditing(false);
//       } catch (fetchErr) {
//         console.error('Failed to refetch testimonials:', fetchErr);
//         setError('Saved, but failed to reload testimonials.');
//       }
//     } catch (err) {
...
//           {Array.isArray(testimonials) && testimonials.filter(t => t.clientName && t.des).length > 0 ? (
//             testimonials.filter(t => t.clientName && t.des).map((item, index) => (
//               <div key={item.testId || index} className="flex items-start gap-4">
...
//                   <p className="text-white font-semibold mb-1 font-mono" style={{ fontWeight: 600, fontSize: 16 }}>{item.clientName || item.name}</p>
//                   <p className="text-gray-300 font-mono" style={{ fontWeight: 400, fontSize: 16 }}>{item.des || item.message}</p>
//                 </div>
